...
export async function getRepoInfo(): Promise<RepoInfo> {
  ...
  try {
    ...
    if (mergeHeadExists) {
      ...
      const rebaseMergeExists = await Bun.file(`${gitDirPath}/rebase-merge`).exists()
      const rebaseApplyExists = await Bun.file(`${gitDirPath}/rebase-apply`).exists()
      if (rebaseMergeExists || rebaseApplyExists) {
        inProgressOperation = 'rebase'
      } else {
        const cherryPickHeadExists = await Bun.file(`${gitDirPath}/CHERRY_PICK_HEAD`).exists()
...
export async function getWorktrees(): Promise<Worktree[]> {
  ...
  for (const block of blocks) {
    ...
    const isAccessible = await Bun.file(path).exists()
...
export function isBranchCheckedOut(
  ...
  return worktrees.some(
    w => w.branchName === branchName && w.isAccessible && w.headState === 'branch'
  )
}
